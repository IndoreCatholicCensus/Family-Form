...
function setupConditionalLogic() {
    ...
    function renderJobSeekers(initialCount = 0) {
        ...
        help.innerHTML = `
          <div id="jobSeekerHint" class="message info hidden" style="margin:0 0 12px; padding: 12px; background: #e0f2fe; border-left: 4px solid #0284c7; border-radius: 6px;">
          <strong style="color: #075985;">📢 Who will be contacted:</strong>
          <p style="margin: 6px 0 0; font-size: 14px; color: #0c4a6e; line-height: 1.5;">
            When a job opening comes up, the parish will contact the Head of Family's mobile number provided in Step 1.
            Please ensure the mobile number is correct and reachable.
          </p>
          <p style="margin: 6px 0 0; font-size: 13px; color: #075985; font-style: italic;">
            रोज़गार के अवसरों की जानकारी मिलने पर पैरिश ऑफिस परिवार के मुखिया के मोबाइल नंबर (Step 1 में दिया गया) पर संपर्क करेगा।
          </p>
         </div>
       ...
       `;
        ...
        jobContainer.querySelector('#addJobSeekerBtn')?.addEventListener('click', () => {
          ...
          // Show hint only when adding the FIRST person
          if (existing === 0) {
            jobContainer.querySelector('#jobSeekerHint')?.classList.remove('hidden');
          }
         ...
         // Remove button handler (using event delegation on persistent container)
            const jobContainer = document.getElementById('jobSeekerContainer');
         if (jobContainer) {
            ...
            const blocksWrap = document.getElementById('jobSeekerBlocks');
            ...
            // Hide hint if no job seekers left
            const remaining = blocksWrap.querySelectorAll('[data-jobseeker]').length;
            if (remaining === 0) {
                const hint = jobContainer.querySelector('#jobSeekerHint');
